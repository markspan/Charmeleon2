...
static void PrintFunction( const char* funcname, const char *result )
{
	...
	if( fp == NULL )
	{
		// Opening of "C:\tmsi.log" doesn't work
		errorcode = fopen_s( &fp, "tmsi_example.log", "w" );

		if( errorcode != 0 )
		{
			printf( "Can not open log file in this directory, program is stopped");
			AppExit(1);
		}
	}
	...
	if (fp != NULL )
	{
		...
		fprintf( fp, "%4d-%2d-%2d-%2d:%2d:%2d.%3d ", Time.wYear, Time.wMonth, Time.wDay, Time.wHour, Time.wMinute, Time.wSecond, Time.wMilliseconds );
		fprintf( fp, funcname );
		fprintf( fp, ", " );
		fprintf( fp, result );
		fprintf( fp, "\r\n" );
...
{
	char *AlarmOn = "Alarm On";
	char *AlarmOff = "Alarm Off";
	char *AlarmState = AlarmOff ;
...
static void SetRecordingSchedule( void *Handle, WORD MinutesOfOffset )
{
	int Status;
	int ErrorCode;
	TMSiRecordingConfigType APISchedule;
	...
	if( MinutesOfOffset == 0 )
	{
		...
		APISchedule.StartControl = (TMSiStartControlType) sc_power_on_record_auto_start;
		strncpy_s( APISchedule.MeasureFileName, MAX_MEASUREFILENAME_LENGTH, "MANREC1", 4 );
	}
	else
	{
		...
		APISchedule.AlarmTimeStart.wMinute += MinutesOfOffset ; // Start at now+2 minutes
		APISchedule.AlarmTimeStop.wMinute = APISchedule.AlarmTimeStart.wMinute+ 1 ; // End 1 minute after start
		APISchedule.AlarmTimeInterval.wHour = 1 ; // repeat every hour
		APISchedule.AlarmTimeCount = 2 ; // repetition is 2
		APISchedule.AlarmTimeInterval.wMinute = 1 ;

		APISchedule.RFInterfStartTime.wMinute = (APISchedule.RFInterfStartTime.wMinute+MinutesOfOffset) % 60 ;
		APISchedule.RFInterfStopTime.wMinute = APISchedule.AlarmTimeStart.wMinute+ 1 ; // End 1 minute after start
		APISchedule.RFInterfInterval.wHour = 1 ; // repeat every hour
		APISchedule.RFInterfCount = 2 ; // repetition is 2

		strncpy_s( APISchedule.MeasureFileName, MAX_MEASUREFILENAME_LENGTH, "TIMREC1", 4 );
	}
...
{
printf("\rSC %8d [%3d]=%4d #%d        ", TotalNrSamples, ShowChannel,SignalBuffer[ShowChannel], NrSamples );
}

static void GetSampleDataWhileMeasuring( void* Handle, unsigned int WhishedLoops, unsigned int ExpectedSawDifference, 
	int ExpectedSawMask, unsigned int DesiredSampleMinutes, int DesiredSampleRate )
{
...
#define USE_RAW_INT
					...
					PrintSaw( Total / BytesPerSample, ShowChannel, SignalBuffer, NrSamples );
#else
					...
					for(i=0 ; i< NrSamples ; i++ )
					{
						...
						ActualSawDifference = (unsigned int) (( CurrentSaw - LastSaw ) & ExpectedSawMask );

						if( ActualSawDifference != ExpectedSawDifference && ActualSawDifference != 0x0 ) 
						{
							Jumps++ ;
							JumpsSaw += ActualSawDifference ;
							sprintf_s( PrintString, LENGTH_PRINTSTRING, "\rSC %d DIFF [%u]=%d (%u)                  ", TotalNrSamples, SawIndex, CurrentSaw, LastSaw );
							TRACEACTION( PrintString );	
						}
		...
		printf( "SCTotal %d SCHits %d SCMin %d SCMax %d\n", SCTotal, SCHits, SCMin, SCMax );
	...
	{	
		...
		{			
			...
			{	
				...
				if( TotalNrSamples >= APIFileHeader.NumberOfSamp )
				{
					sprintf_s( PrintString, LENGTH_PRINTSTRING, "Got %d samples from %d in file", TotalNrSamples, APIFileHeader.NumberOfSamp );
					TRACEACTION( PrintString );
					Sampling = 0 ;
				}
...
static void GetCalibrationData( void* Handle )
{
	ULONG SampleRateInMilliHz, SampleRateInHz ;
	ULONG SignalBufferSizeInSamples;
	int BytesReturned;
	...
	// This loop demonstrate that you can set several different Volt values
	// If you want just one specific value, remove the loop and use that specific value
	for( Volt= IC_VOLT_050 ; Volt < IC_VOLT_500 ; Volt++ )
	{
		// Turn on the calibration mode
		if( fpSetMeasuringMode( Handle, MEASURE_MODE_CALIBRATION_EX, Volt ))
		{
			sprintf_s( PrintString, LENGTH_PRINTSTRING, "Measuremode calibration set to %d", Volt ); 
			TRACEACTION( PrintString );
		}
		else
		{	
			ErrorCode = fpGetErrorCode(Handle);
			sprintf_s( PrintString, LENGTH_PRINTSTRING, "Unable to set Measuremode calibration, errorcode = %d (%s)", 
				ErrorCode, fpGetErrorCodeMessage(Handle,ErrorCode) );
			TRACEACTION( PrintString );
			return;
		}
		...
		{			
			...
			{	
				...
				Total += BytesReturned; 
				printf("\rSC %8d CAL %3d %3d %3d %3d #%d ", Total / BytesPerSample , SignalBuffer[0], SignalBuffer[1], SignalBuffer[2], SignalBuffer[3], NrSamples );
			}
			else 
			{
				if( BytesReturned == 0 )
		...
		// Turn off the calibration mode, and go back to normal (=sampling) mode
		if( fpSetMeasuringMode( Handle, MEASURE_MODE_NORMAL, 0 ))
		{
			TRACEACTION("Measuremode normal set");
		}
...
{	
	...
	TRACEACTION( "ExampleCode.exe $Revision: 132 $" );

	if( argc  < 2  )
	{
		TRACEACTION("Usage: example (w=WLAN, u=USB b=Bluetooth n=Network) s=sample [loops][sawtoothdiff][sawtoothmask][loopminutes][samplerate], i=impedance, c=calibration, f=file t=timed recording [s = set, g = get] [minute offset]");
		AppExit(1);
	}	
...
#ifdef _DEBUG
	{
		TCHAR Hardcoded[255] = _T("D:\\SCM\\fiber64_svn130_dev\\TmsiApi\\Debug\\TMSiSDK.dll"); 
	...
	for(OpenCloseLoop=0 ;OpenCloseLoop < 1 ; OpenCloseLoop++ )
	{
		...
		if( DeviceList !=NULL && DeviceList[0] != NULL )
		{
			...
			if( *argv[1] == 'b' )
			{
				//DeviceLocator = "NeXus-10 0928070173";
				//DeviceLocator = "Porti7-16b8at 0207090010" ;
				//DeviceLocator = "Porti7-16b 0207080004";
				//DeviceLocator = "Porti_314khz";
			}

			if( *argv[1] == 'w' )
			{
				//DeviceLocator = "Mobita7-16b 0787080005";
			}
		...
		else
		{
			...
			TRACEACTION( PrintString );
			sprintf_s( PrintString, LENGTH_PRINTSTRING, "CurrentBlockType %d", TMSiExtFrontEndInfo.CurrentBlockType );
			TRACEACTION( PrintString );
			...
			sprintf_s( PrintString, LENGTH_PRINTSTRING, "BatteryStatus.AccumCurrent %d mA", TMSiBatReport.AccumCurrent );
			TRACEACTION( PrintString );
			...
			TRACEACTION( PrintString );
			sprintf_s( PrintString, LENGTH_PRINTSTRING, "TMSiDeviceReport.AdapterCycles %d",  TMSiDeviceReport.AdapterCycles );
			TRACEACTION( PrintString );
			sprintf_s( PrintString, LENGTH_PRINTSTRING, "TMSiDeviceReport.AdapterStatus %d",  TMSiDeviceReport.AdapterStatus );
			TRACEACTION( PrintString );
			sprintf_s( PrintString, LENGTH_PRINTSTRING, "TMSiDeviceReport.MobitaCycles %d",  TMSiDeviceReport.MobitaCycles);
			TRACEACTION( PrintString );
		...
		if( argc > 2 )
		{
			if( *argv[2] == 's' )
			{
				...
				if( argc>4 )
					ExpectedSawDifference = atoi(argv[4]);

				if( argc>5 )
					ExpectedSawMask = atoi(argv[5]);
			...
			if( *argv[2] == 'c' )
			{
				sprintf_s( PrintString, LENGTH_PRINTSTRING, "Use calibration measurement" );
				TRACEACTION( PrintString );
			...
			if( *argv[2] == 't' )
			{
				...
				if( argc>3 )
				{
					if( *argv[3] == 's' )
					{
						...
						SetRecordingSchedule( Handle, MinutesOfOffset );
					}
